...
const UsersPage = () => {
  ...
  return (
    <div>
      <h1>Users (Page {currentPage}, Sort: {sortBy})</h1>
...
const UserProfile = () => {
  ...
  return (
    <div>
      <button onClick={() => saveProfile(data)}>
        Save Profile
      </button>
    </div>
  );
...
<RouteGuard 
  route={route}
  forbiddenComponent={CustomErrorPage}
>
  <YourContent />
</RouteGuard>
```
...
// Page components
const HomePage = () => <div><h1>Welcome Home</h1></div>;
...
const UsersPage = () => <div><h1>Users Management</h1></div>;
const UserDetailPage = () => {
  const { id } = useParams();
  return <div><h1>User {id} Details</h1></div>;
};
...
```jsx
// New approach
import { Router, Navigation, Routes, initializeAuth, createRouteConfig } from 'react-auth-router';
...
const routeConfig = createRouteConfig({
  ...
  protected: [
    { path: '/dashboard', component: 'DashboardPage', title: 'Dashboard', requireAuth: true }
  ]
});
...
export const handleApiError = (error, context = '') => {
  ...
  return {
    ...mappedError,
    message: context ? `${context}: ${mappedError.message}` : mappedError.message,
...
initializeAuth({
  onStateChange: (authState) => {
    ...
    if (authState.user) {
      store.dispatch(appSlice.actions.setTheme(authState.user.preferences?.theme || 'light'));
    }
  }
});
```
...
initializeAuth({
  onStateChange: (authState) => {
    if (authState.type === 'LOGIN_SUCCESS') {
      useAppStore.getState().setTheme(authState.user.preferences?.theme || 'light');
    }
  }
});
```
...
describe('Protected Component', () => {
  test('renders when user has permission', () => {
    const mockStore = createMockAuthStore({
      user: { id: 1, name: 'Test', roles: ['admin'], permissions: ['read_users'] }
    });
